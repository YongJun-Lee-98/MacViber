...
extension TerminalView {
    ...
    func mapColor (color: Attribute.Color, isFg: Bool, isBold: Bool, useBrightColors: Bool = true) -> TTColor
    {
        ...
        case .defaultColor:
            if isFg {
                debugLogger?("[SWIFTTERM] mapColor(.defaultColor, isFg=true) → \(nativeForegroundColor)")
    ...
    func colorsChanged ()
    {
        debugLogger?("[SWIFTTERM] colorsChanged() - clearing caches, nativeFg=\(nativeForegroundColor)")
        urlAttributes = [:]
        attributes = [:]
    ...
    func getAttributes (_ attribute: Attribute, withUrl: Bool) -> [NSAttributedString.Key:Any]?
    {
        ...
        // Cache miss - will compute fresh attributes
        if case .defaultColor = fg {
            debugLogger?("[SWIFTTERM] getAttributes cache MISS for .defaultColor, will call mapColor")
        }
    ...
    func buildAttributedString (row: Int, line: BufferLine, cols: Int, prefix: String = "") -> ViewLineInfo
    {
        ...
        for col in 0..<cols {
            ...
            // Skip placeholder cells for wide characters (Korean, CJK, etc.)
            // Only append actual characters, not the empty continuation cells
            if ch.code != 0 {
        ...
        res.append (NSAttributedString(string: str, attributes: finalAttrs))

        // Debug: log the foreground color in the final attributed string
        if row == 0, let fgColor = finalAttrs?[.foregroundColor] {
            debugLogger?("[SWIFTTERM] buildAttributedString row=0 finalFgColor=\(fgColor)")
        }
        ...
        for row in firstRow...lastRow {
            ...
            for run in CTLineGetGlyphRuns(ctline) as? [CTRun] ?? [] {
                ...
                var positions = runGlyphs.enumerated().map { (i: Int, glyph: CGGlyph) -> CGPoint in
                    // Use cellPositions array to get correct position for wide characters (Korean, CJK)
                    let cellPos = (col + i) < lineInfo.cellPositions.count ? lineInfo.cellPositions[col + i] : col + i
                    return CGPoint(x: lineOrigin.x + (cellDimension.width * CGFloat(cellPos)), y: lineOrigin.y + yOffset)
                }
                ...
                // Set text drawing mode to fill to ensure fill color is used for glyphs
                ...
                if runAttributes.keys.contains(.foregroundColor) {
                    ...
                    if row == 0 && col == 0 {
                        debugLogger?("[SWIFTTERM] DRAW: setting fillColor to \(color)")
                    }
                    context.setFillColor(cgColor)
                } else {
                    if row == 0 && col == 0 {
                        debugLogger?("[SWIFTTERM] DRAW: no .foregroundColor, using nativeFg=\(nativeForegroundColor)")
                    }
