...
open class TerminalView: NSView, NSTextInputClient, NSUserInterfaceValidations, TerminalDelegate {
    ...
    /// Debug logger callback for tracing rendering pipeline
    ...
    public var nativeForegroundColor: NSColor {
        ...
        set {
            if settingFg { return }
            settingFg = true
            debugLogger?("[SWIFTTERM] nativeForegroundColor setter: \(newValue)")
            _nativeFg = newValue
            terminal.foregroundColor = nativeForegroundColor.getTerminalColor ()
            colorsChanged()
            debugLogger?("[SWIFTTERM] nativeForegroundColor setter done, _nativeFg=\(_nativeFg)")
            settingFg = false
        }
    }
